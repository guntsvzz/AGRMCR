...
class BinaryRecommendEnvPGPR(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0, domain = 'Office_Products'):
        ...
        self.feature_length = getattr(self.dataset, 'category').vocab_size
        self.user_length = getattr(self.dataset, 'user').vocab_size
        self.item_length = getattr(self.dataset, 'item').vocab_size 
        self.domain = domain
        ...
        print('ui_dict',len(self.ui_dict))
        ...
        if embeds:
            embeds['ui_emb'] = np.concatenate((embeds['user'], embeds['item']), axis=0)
            print("Number of User & Item", embeds['ui_emb'].shape)
            self.ui_embeds = embeds['ui_emb']
            self.feature_emb = embeds['category']
        else:
            self.ui_embeds = nn.Embedding(self.user_length+self.item_length, 100).weight.data.numpy()
            self.feature_emb = nn.Embedding(self.feature_length, 100).weight.data.numpy()
    ...
    def __load_rl_data__(self, data_name, mode):
        ...
        #     mydict[key] = [item for item in interact if not isinstance(item, str)]
    ...
    def __user_dict_init__(self):   #Calculate the weight of the number of interactions per user
        ...
        print("user weight shape", len(self.user_weight_dict))
    ...
    def reset(self, embed=None):
        ...
        print('user_like_fea', self.kg.G['item'][self.target_item]['category_of'])
        user_like_random_fea = random.choice(self.kg.G['item'][self.target_item]['category_of']) #belong_to -> category_of
    ...
    def _get_cand(self):
        if self.random_sample_feature:
            ...
            cand_feature = self._map_to_all_id(self.reachable_feature[:self.cand_num],'feature') #GO here
        if self.random_sample_item:
            ...
            cand_item = self._map_to_all_id(self.cand_items[:self.cand_item_num],'item') #GO here
    ...
    def _updata_reachable_feature(self):
        ...
        for cand in self.cand_items:
            fea_belong_items = list(self.kg.G['item'][cand]['category_of']) # A-I #belong_to -> category_of
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            ...
            prefer_embed = self.feature_emb[self.user_acc_feature, :]  #np.array (x*64)
            unprefer_feature = list(set(self.user_rej_feature) & set(self.kg.G['item'][item_id]['category_of'])) #belong_to -> category_of
    ...
    def _ask_update(self, asked_feature):
        ...
        feature_groundtrue = self.kg.G['item'][self.target_item]['category_of'] #belong_to -> category_of
    ...
    def _update_cand_items(self, asked_feature, acc_rej):
        if acc_rej:    # accept feature
            ...
            feature_items = self.kg.G['category'][asked_feature]['category_of'] #fix feature #belong_to -> category_of
    ...
    def _update_feature_entropy(self):
        if self.ent_way == 'entropy':
            cand_items_fea_list = []
            for item_id in self.cand_items:
                cand_items_fea_list.append(list(self.kg.G['item'][item_id]['category_of'])) #belong_to -> category_of
            ...
            for score_ind, item_id in enumerate(self.cand_items):
                cand_items_fea_list = list(self.kg.G['item'][item_id]['category_of']) #belong_to -> category_of
