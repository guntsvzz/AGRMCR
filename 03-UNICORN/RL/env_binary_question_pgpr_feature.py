...
class BinaryRecommendEnvPGPR(object):
    def __init__(self, kg, dataset, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0, domain = 'Office_Products'):
        ...
        self.feature_length = getattr(self.dataset, 'brand').vocab_size + getattr(self.dataset, 'category').vocab_size
        ...
        if embeds:
            ...
            self.feature_emb = np.concatenate((embeds['brand'], embeds['category']), axis=0) #embeds['feature'] 
        else:
    ...
    def _updata_reachable_feature(self):
        ...
        for cand in self.cand_items:
            fea_belong_items = list(self.kg.G['item'][cand]['feature']) # A-I #belong_to -> category_of
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            ...
            prefer_embed = self.feature_emb[self.user_acc_feature, :]  #np.array (x*64)
            unprefer_feature = list(set(self.user_rej_feature) & set(self.kg.G['item'][item_id]['feature'])) #feature=belong_to+category_of
    ...
    def _recommend_update(self, recom_items):
        ...
        # print('set(recom_items)', set(recom_items))
        # print('set(self.cand_items[: self.rec_num])', set(self.cand_items[: self.rec_num]))
    ...
    def _update_feature_entropy(self):
        if self.ent_way == 'entropy':
            cand_items_fea_list = []
            for item_id in self.cand_items:
                cand_items_fea_list.append(list(self.kg.G['item'][item_id]['feature']))
            ...
            for score_ind, item_id in enumerate(self.cand_items):
                cand_items_fea_list = list(self.kg.G['item'][item_id]['feature']) 
            ...
            self.attr_ent = [0] * self.attr_state_num  # reset attr_ent
            real_ask_able = list(set(self.reachable_feature) & set(self.attr_count_dict.keys())) #intersection
            ...
            for fea_id in real_ask_able:
                ...
                if p1 == 1 or p1 <= 0:
                    ...
                    ent = (- p1 * np.log2(p1) - p2 * np.log2(p2))
                    # print('fea_id',fea_id)
                    # print('len of attr_ent',len(self.attr_ent))
                    self.attr_ent[fea_id] = ent
