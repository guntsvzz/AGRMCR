...
def main():
	...
	user_set = set()
	product_set = set()
	with gzip.open(f'./raw_data/{review_file}_5.json.gz', 'rt') as g:
		for line in tqdm(g):
			l = json.loads(line) # l = eval(l)
			user = l.get('reviewerID', '')
			product = l.get('asin', '')
			# Debugging outputs to check the data
	...
	with gzip.open(output_path + 'review_text.txt.gz', 'wt', encoding='utf-8') as fout_text, \
		...
		gzip.open(f'./raw_data/{review_file}_5.json.gz', 'rt', encoding='utf-8') as g:
		index = 0
		# for l in g:
		# 	l = eval(l)
		for line in tqdm(g):
