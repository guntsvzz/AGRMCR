...
def fill_node_features(node_data, node_to_num_id, valid_node_asin):
  ...
  for eachline in tqdm(meta_file):
    ...
    if asin in valid_node_asin:
      ...
      if key in mapping:
          if value not in mapping[key]:  # Check if the value is not already in the list
              mapping[key].append(value)  # If yes, append the value to the existing list
      else:
          mapping[key] = [value]  # If not, create a new list with the value as the first element

  # Save the mapping to a JSON file
  with open(f'./tmp/{data_name}_c2t.json', 'w') as file:
