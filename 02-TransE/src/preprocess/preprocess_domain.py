...
def get_rating(user_idx, product_id, rating_profile):
    # Correct way to filter the DataFrame
    filtered_rating = rating_profile[(rating_profile['user'] == user_idx) & (rating_profile['item'] == product_id)]
    return filtered_rating.rating

def make_like_and_dislike(config, purchases, p_brand, nb_users, rating_profile):
    """Create the 'user like brand' and 'user dislike brand' relations and save them in like_u_b.txt and dislike_u_b.txt."""

    like_b_u = defaultdict(Counter)
    dislike_b_u = defaultdict(Counter)

    for user, products in tqdm(purchases.items()):
        for product in products:
            product_id = int(product)
            rating = get_rating(user, product_id, rating_profile).values[0]
            if rating is not None:
                if rating >= 3:
                    for brand in p_brand[product_id]:
                        if brand != "-1":
                            like_b_u[user][brand] += 1
                else:
                    for brand in p_brand[product_id]:
                        if brand != "-1":
                            dislike_b_u[user][brand] += 1
            

    # # Rank the liked brands by frequency
    # for user in like_b_u:
    #     like_b_u[user] = like_b_u[user].most_common()

    # # Rank the disliked brands by frequency
    # for user in dislike_b_u:
    #     dislike_b_u[user] = dislike_b_u[user].most_common()

    # like_b_u_file = os.path.join(config.processed_data_dir, "like_u_b_rate.txt")
    # dislike_b_u_file = os.path.join(config.processed_data_dir, "dislike_u_b_rate.txt")

    # with open(like_b_u_file, "w") as f:
    #     for i in range(nb_users):
    #         f.write(" ".join([brand for brand, _ in like_b_u[i]]) + "\n")

    # with open(dislike_b_u_file, "w") as f:
    #     for i in range(nb_users):
    #         f.write(" ".join([brand for brand, _ in dislike_b_u[i]]) + "\n")
    
    # Adjust the like and dislike counts based on most common occurrence
    final_like_b_u = defaultdict(list)
    final_dislike_b_u = defaultdict(list)

    for user in tqdm(range(nb_users)):
        user_likes = like_b_u[user]
        user_dislikes = dislike_b_u[user]
        
        all_brands = set(user_likes.keys()).union(set(user_dislikes.keys()))
        
        for brand in all_brands:
            like_count = user_likes[brand]
            dislike_count = user_dislikes[brand]
            
            if like_count >= dislike_count:
                final_like_b_u[user].append(brand)
            else:
                final_dislike_b_u[user].append(brand)
    ...
    with open(like_b_u_file, "w") as f:
        for i in range(nb_users):
            f.write(" ".join(final_like_b_u[i]) + "\n")

    with open(dislike_b_u_file, "w") as f:
        for i in range(nb_users):
            f.write(" ".join(final_dislike_b_u[i]) + "\n")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help="Config file.",
        default="config/beauty/graph_reasoning/preprocess.json",
    )
    ...
    # Create the "user like brand" relation and save it in like_u_b_rate.txt
    # Create the "user dislike brand" relation and save it in dislike_u_b_rate.txt
    rating_file = os.path.join(config.original_data_dir, "rating_u_p_r.txt")
    rating_profile = pd.read_csv(rating_file, delimiter=' ', names=['user','item','rating'])
    make_like_and_dislike(config, purchases, p_brand, nb_users, rating_profile)
