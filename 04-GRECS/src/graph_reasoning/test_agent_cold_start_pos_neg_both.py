...
def batch_beam_search_cold_start(
    env, model, kg_config, uids, device, topk=[10, 3, 1], policy=0, user_pref_embed=None
    ):
...
def predict_paths(
    policy_file, path_file, config, config_agent, kg_config, args, set_name="test",
):
    ...
    if set_name in ['test', 'test_cold_start', 'test_cold_start_trim']:
        print(f'{set_name}')
        batch_size = 16
        start_idx = 0
        all_paths, all_probs = [], []
        ...
        init_embed = InitalUserEmbedding(
            set_name="test", config=config, preference = args.preference
        )
        for idx in tqdm(range(len(user_pref))):
            ...
            #other past history of user preference
            # idx_cand_user, cand_user_emb = init_embed.distance(
            #     user_pref_emb, top_k=args.topk_candidate+1) #N+1 because it wil remove later
            
            pos_pref_embed, neg_pref_embed = init_embed.translation(
                user_acc_feature=user_acc_feature, 
                user_rej_feature=user_rej_feature,
                user_rej_items = user_rej_items, 
            )
            ...
            #new one
            idx_cand_user, cand_user_emb = init_embed.distance(
                pos_pref_embed=pos_pref_embed, 
                neg_pref_embed=neg_pref_embed, 
                top_k=args.topk_candidate #+1, 
                # preference=args.preference
            )
            
            user_preferred['related_user'] = idx_cand_user.tolist()
            ...
            # break
        print('All cold-start user preference', len(cold_start_uids))
        #####################################################################################################
        ...
        for uid in tqdm(cold_start_uids):
            ...
            paths, probs = batch_beam_search_cold_start(
                ...
                topk=config_agent.topk,
                policy=config_agent.modified_policy,
                user_pref_embed = pos_pref_embed #adding user_pref only positive side 
            )
